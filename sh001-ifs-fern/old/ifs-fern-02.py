...
fig, ax = plt.subplots(figsize=(9,16), dpi=1920/16)
plt.axis('off')
ax.set_xlim(-3, 3)
ax.set_ylim(0, 11)
ax.set_aspect('equal')
...
scat = ax.scatter(liste_x[0], ylift+liste_y[0], c="k", s=0.3)
...
ani = animation.FuncAnimation(fig=fig, func=update, frames=330, interval=1, repeat=False)

#### ani.save(filename="short_ifs_01.mp4", writer="ffmpeg")

ffmpeg_writer = animation.FFMpegWriter(fps=30, bitrate=-1, codec='libx264')
ani.save('sh001-ifs-fern.mp4', writer=ffmpeg_writer)
